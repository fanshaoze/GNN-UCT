...
class TopoGenSimulator(uct.Simulator):
    def __init__(self, _configs, _approved_path_freq, _component_condition_prob, _key_expression,
                 _key_sim_effi={}, _fix_para_set=None,
                 _num_component=4,
                 target=None):
    ...
    def update_action_set(self):
        ...
        if len(self.current.component_pool) == 3:
            ...
            if self.current.graph_is_valid():
                self.act_vect.append(TopoGenAction('finish_topo', 0))
            ...
            param = json.load(open("./UCT_for_CD_no_sweep/SimulatorAnalysis/param.json"))
    ...
    def get_reward(self):
        # For parameter, we only consider Duty Cycle.
        # Duty cycle will be assigned as -1 for the topologies which are invalid
        ...
        if self.graph_2_reward.__contains__(key + '$' + str(self.current.parameters)):
            ...
            if para_result:
                ...
                if para_result:
                    # Note that, the duty cycle is set as 'None' from the analytics result, not -1
    ...
    def simulate_one_analytics_result(self, analytics_info):
        ...
        for fn in analytic:
            ...
            for param in analytic[fn]:
                ...
                # param_value = []
                # print(param)
                # for value in param[1:-1].split(', '):
                #     value.replace('\'','')
                #     param_value.append(float(value))
    ...
    def current_sim_reward(self):
        ...
        if self.current.graph_is_valid():
            results_tmp = get_analytics_file(self.key_expression, self.current.graph,
                                             ...
                                             [self.current.parameters], self.configs_['min_vout'])
        else:
            results_tmp = None
    ...
    def get_single_topo_sim_result(self, current):
        ...
        if self.key_sim_effi_.__contains__(current_key + '$' + str(current.parameters)):
            effi_info = self.key_sim_effi_[current_key + '$' + str(current.parameters)]
            print('find pre simulated &&&&&&&&&&&&&&&&&&&&&&')
            effi_info = {current.parameters: {'efficiency': effi_info[0], 'Vout': effi_info[1]}}
        else:
            effi_info = self.simulate_one_analytics_result(results_tmp)
            saved_effi_info = {}
        ...
        return effi_info[topk_max_para]['efficiency'], effi_info[topk_max_para]['Vout'], topk_max_para
    ...
    def find_simu_max_reward_para(self, para_effi_info):
        ...
        for para in para_effi_info:
            ...
            tmp_reward = calculate_reward(effi, self.configs_['target_vout'],
                                          self.configs_['min_vout'], self.configs_['max_vout'])
    ...
    def is_terminal(self):
        # + 1 means the last phase is set parameters. Every action is a parameter combination
        # Duty Cycle: [0.1,0.2]
        # Rout: [1,100]
        # actions: (duty cycle, Rout): (0.1,1)(0.1,100)(0.2,1)(0.2,100)
        if self.current.step >= (len(self.current.component_pool) - 3) + len(self.current.port_pool) + 1:
            return True
        return False
