...
def key_expression_dict(target_vout_min=-500, target_vout_max=500):
    data_json_file = json.load(open("./UCT_for_CD_no_sweep/SimulatorAnalysis/database/data.json"))
    # data_json_file = json.load(open("./database/data.json"))
    # print(len(data_json_file))
    expression_json_file = json.load(open("./UCT_for_CD_no_sweep/SimulatorAnalysis/database/expression.json"))
    ...
    simu_results = []
    with open("./UCT_for_CD_no_sweep/SimulatorAnalysis/database/analytic.csv", "r") as csv_file:
    ...
    with open('./UCT_for_CD_no_sweep/SimulatorAnalysis/database/analytic-expression.json', 'w') as f:
...
def generate_para_strs(device_list, duty_cycle):
    para_strs = []
    parameters = json.load(open("./UCT_for_CD_no_sweep/SimulatorAnalysis/param.json"))
    if duty_cycle != -1:
...
def get_one_device_list(graph, comp2port_mapping, port2comp_mapping, idx_2_port, port_2_idx, parent, component_pool,
    ...
    directory_path = './UCT_for_CD_no_sweep/SimulatorAnalysis/components_data_random'
    target_folder = './UCT_for_CD_no_sweep/SimulatorAnalysis/database'
...
def simulate_one_analytics_result(analytics_info):
    ...
    cki_folder = '/UCT_for_CD_no_sweep/SimulatorAnalysis/database/cki/'
    for fn in analytic:
        print(int(fn[6:9]))
        count = 'tmp_simu'
...
def read_analytics_result():
    data_json_file = json.load(open("./UCT_for_CD_no_sweep/SimulatorAnalysis/database/analytic-expression.json"))
...
def read_no_sweep_analytics_result():
    data_json_file = json.load(
        open("./UCT_for_CD_no_sweep/SimulatorAnalysis/database/no-sweep-analytic-expression.json"))

    return data_json_file


def read_sim_result():
    data_json_file = json.load(open("./UCT_for_CD_no_sweep/SimulatorAnalysis/database/key-sim-result.json"))
    return data_json_file


def read_no_sweep_sim_result():
    data_json_file = json.load(open("./UCT_for_CD_no_sweep/SimulatorAnalysis/database/no-sweep-key-sim-result.json"))
    return data_json_file
...
def save_no_sweep_analytics_result(simu_results):
    with open('./UCT_for_CD_no_sweep/SimulatorAnalysis/database/no-sweep-analytic-expression.json', 'w') as f:
        json.dump(simu_results, f)
    f.close()


def save_sim_result(simu_results):
    with open('./UCT_for_CD_no_sweep/SimulatorAnalysis/database/key_sim_result.json', 'w') as f:
        json.dump(simu_results, f)
    f.close()


def save_no_sweep_sim_result(simu_results):
    with open('./UCT_for_CD_no_sweep/SimulatorAnalysis/database/no-sweep-key-sim-result.json', 'w') as f:
        json.dump(simu_results, f)
    f.close()
